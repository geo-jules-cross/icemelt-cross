...
!---------------------------------------------------------------------
    ...
    real day_melt_abl_out(7,7000) ! Changed by JMC from line below
...
! polynomial of albedo during the summer months (~Days 75-265 *JMC edit) 
! for an ice-exposed surface.  
! (from 13 summers of TAR, with snow manually removed)
...
! JMC commented this out to experiment with snow
        itemp=365*(iter/365)    ! MJH: integer div will round down (FLOOR)
        ...
        if ((albedo .lt. snow_albedo_thresh).or.(iscliff.eq.1)) then
            ...
            snow_cover_depth_old = 0.05 !constant 5 cm - any positive
        endif

!       snow_cover_depth_old = 0.0 !just make it 0 instead
!       snow_cover_depth_old = 0.05 !just make it 0.05 instead (JMC)
