...
**Code:** `icemelt_spatial_jmc.f95`  
**File Location:**
`I:\Antarctica_Julian\icemelt-jmc-working`  
**Date of README:** 28 June 2018  
...
- **July 2018:** JC added new albedo method to account for spatial variability of albedo.
...
`gfortran -o ./icemelt ./icemelt_spatial_jmc.f95`

In this command the `-o` option specifies that the `./icemelt_spatial_jmc.f95` program is compiled to an executable object at `./icemelt` (rather than the default `a.out`). This executable is used in further shell scripts to run the model.
...
- Once connected to the `rc.pdx.edu` server, files can be copied from `I:\Antarctica_Julian\icemelt-jmc-working` via SSH Secure Copy.
...
- Files can be copied from `I:\Antarctica_Julian\icemelt-jmc-working` via SSH Secure Copy.
- For example, to copy the icemelt directory from the `I:` drive to the current directory use:  
`scp -r USERNAME@sftp.myfiles.pdx.edu:Idrive-ResourcesFolder/Research/Shares/antarctica/WORKING_DIRECTORY .`
