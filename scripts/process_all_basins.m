...
%% Post-process Multi-surface Run for Lake Model
...
% Model runs to 02/01/2012 (But skip last day?)
...
for thebasin=1:35 % 34 % 36
    numcells(thebasin)=sum(sum(basins==basinkey(thebasin)));
end

stream_inds=[];
for i=1:length(basins2process)
    stream_inds=[stream_inds; find(streamkey==basins2process(i))]; %#ok<*AGROW>
end

basin_inds=[];
for i=1:length(basins2process)
    basin_inds=[basin_inds; find(basinkey==basins2process(i))]; %#ok<*AGROW>
end
...
streams.ErrUpr=streams.TDQ+(streams.TDQ.*error.Error);
streams.ErrLwr=streams.TDQ-(streams.TDQ.*error.Error);
dates = streams.dates;
...
% start off by assuming a full size grid cell everywhere.
...
uniformsurface.surfTDQ=zeros(ndays,36);
uniformsurface.surfABL=zeros(ndays,36);
uniformsurface.subTDQ=zeros(ndays,36);
uniformsurface.cellmeltsummer=zeros(nyears,nx,ny)*NaN;
uniformsurface.cellablsummer=zeros(nyears,nx,ny)*NaN;
uniformsurface.cellsubmeltsummer=zeros(nyears,nx,ny)*NaN;
uniformsurface.cellsubmeltSurfacedsummer=zeros(nyears,nx,ny)*NaN;

% Basin Walls
basinwall.surfTDQ=zeros(ndays,36);
basinwall.surfABL=zeros(ndays,36);
basinwall.subTDQ=zeros(ndays,36);
...
%Basin Floors
basinfloor.surfTDQ=zeros(ndays,36);
basinfloor.surfABL=zeros(ndays,36);
basinfloor.subTDQ=zeros(ndays,36);
...
% Cliffs
cliffTDQ=zeros(ndays,36);
cliffs.surfTDQ=zeros(ndays,36);
cliffs.surfABL=zeros(ndays,36);
cliffs.subTDQ=zeros(ndays,36);
cliffarea=zeros(size(basinkey));
cliffcells=zeros(size(basinkey));
cliffMnAnnlMelt=zeros(size(basinkey));
totalcliffarea=zeros(1,36);
cliff.cellmeltsummer=zeros(length(yearIndices),nx,ny)*NaN;
cliff.cellsubmeltsummer=zeros(length(yearIndices),nx,ny)*NaN;

% Totals
cryoholeTDQ=zeros(ndays,36);
surfTDQ=zeros(ndays,36);
subTDQ=zeros(ndays,36);
modelTDQ=zeros(ndays,36); %#ok<*PREALL>

% Other
surfarea=zeros(size(basinkey));
surfcells=zeros(size(basinkey));
surfMnAnnlMelt=zeros(size(basinkey));
...
for i=1:nx
    for j=1:ny
        ...
        if ismember(basins(ny-j+1,i),basins2process)  %y index is backward from the ascii file
            ...
            for yy=years2run
                % Calculate summer (Nov-Feb) season totals for all cells every year
                ...
                uniformsurface.cellmeltsummer(yy-1994,i,j)=sum(M(seasonstart:seasonend,1));
                uniformsurface.cellablsummer(yy-1994,i,j)=sum(M(seasonstart:seasonend,2));
                uniformsurface.cellsubmeltsummer(yy-1994,i,j)=sum(M(seasonstart:seasonend,3));
                
                % Find depth to query the subsurface densities over
                affectedlayers=interp1(subdepths,(1:length(subdepths)),uniformsurface.cellablsummer(yy-1994,i,j)/100);
                temp=0;
                ...
                uniformsurface.cellsubmeltSurfacedsummer(yy-1994,i,j)=temp*100;  % save it
            end
            
            %Apportion this melt to the proper basin
            basinnum=find(basinkey==basins(ny-j+1,i));
            if length(basinnum==1)  % Not all cells belong to basins
                ...
                uniformsurface.surfABL(:,basinnum)=uniformsurface.surfABL(1:ndays,basinnum) + uniformsurface_area(ny-j+1,i)*M(1:ndays,2)/100;
                % Smooth Sub Melt
                ...
                for yy=years2run
                    ...
                    cryoholeTDQ(seasonstart:seasonend,basinnum)=cryoholeTDQ(seasonstart:seasonend,basinnum) + cryohole_area(ny-j+1,i) * uniformsurface.cellsubmeltSurfacedsummer(yy-1994,i,j)/100/(seasonend-seasonstart+1); % just spread it evenly over the season...
                end
            end
        end
    end
...
cellcryoholesummer = max(uniformsurface.cellablsummer, uniformsurface.cellablsummer*0.0);
        ...
        for j=1:ny
            ...
            if ismember(basins(ny-j+1,i),basins2process)  %y index is backward from the ascii file
                ...
                if length(basinnum==1)  %&&(basin_regions(ny-j+1,i)>=0)  %Not all cells belong to basins, check to make sure this is a basin-y part of the drainage basin
                    ...
                    % Basin Surface Ablation
                    basinwall.surfABL(:,basinnum)=basinwall.surfABL(1:ndays,basinnum) + adjFactor*basin_walls_area(ny-j+1,i)*M(1:ndays,2)/100;
                    % Basin Sub-surface Ablation
        ...
        for j=1:ny
            ...
            if ismember(basins(ny-j+1,i),basins2process)  %y index is backward from the ascii file
                ...
                if length(basinnum==1)% Not all cells belong to basins, check to make sure this is a basin-y part of the drainage basin
                    ...
                    % Basin Surface Ablation
                    basinfloor.surfABL(:,basinnum)=basinfloor.surfABL(1:ndays,basinnum) + adjFactor*basin_floor_area(ny-j+1,i)*M(1:ndays,2)/100;
                    % Basin Sub surface Melt
    ...
    for i=1:nx
        ...
        for j=1:ny
            ...
            if ismember(basinsCliff(ny-j+1,i),basins2process)  %y index is backward from the ascii file
                ...
                %Apportion this melt to the proper basin
                basinnum=find(basinkey==basinsCliff(ny-j+1,i));
                if length(basinnum==1)  %Not all cells belong to basins
                    ...
                    % Cliff Surface Ablation
                    cliffs.surfABL(:,basinnum)=cliffs.surfABL(1:ndays,basinnum) + M(1:ndays,2)/100*cliffLength(ny-j+1,i)*cliffHeight;
...
for y=yearIndices
    %Summer melt
    domainsummermelt(y,1)=nansum(nansum(nansum(  squeeze(uniformsurface.cellmeltsummer(y,:,:)/100)'.* flipud(uniformsurface_area)   )));
    domainsummermelt(y,2)=nansum(nansum(nansum(  squeeze(uniformsurface.cellsubmeltsummer(y,:,:)/100)'.* flipud(uniformsurface_area)   )));
...
% Calculate Total Ablation for each season
modelABL=uniformsurface.surfABL+cliffs.surfABL+basinfloor.surfABL+basinwall.surfABL; 

% Loop through each basin
s = 0; %stream counter
for b=[1:length(basinkey)]
    if (ismember(basinkey(b),streamkey))
        s = s +1;
    end
    for yr=years2run
        ...
        % Sum annaul stream discharge
        if (ismember(basinkey(b),streamkey))
            seasonDischarge = nansum(streams.TDQ(strt:fnsh,s));
            if size(seasonDischarge) == 1
                streamYrVol(yr-1994,s)=seasonDischarge;
            else
                streamYrVol(yr-1994,s)=NaN;  % SHOULD THIS BE NAN?
            end
            measdays=find( ~isnan(streams.TDQ(:,s)) & ([1:length(streams.dates)]'>=strt) & ([1:length(streams.dates)]'<=fnsh) );  % note days that have data only
        end
        % Model totals
        ...
        subYrVol(yr-1994,b)=sum(modelTDQsubonly(strt:fnsh,b));
        % Summer
        surfSmVol(yr-1994,b)=sum(modelTDQsurfonly(sumrstart:sumrfnsh,b));
        subSmVol(yr-1994,b)=sum(modelTDQsubonly(sumrstart:sumrfnsh,b));
        
        % Surface Type Totals
        cliffYrVol(yr-1994,b)=sum(cliffTDQ(strt:fnsh,b));
        smoothYrVol(yr-1994,b)=sum(uniformsurface.surfTDQ(strt:fnsh,b) + uniformsurface.subTDQ(strt:fnsh,b));
        basinYrVol(yr-1994,b)=sum(basinfloor.surfTDQ(strt:fnsh,b) + basinfloor.subTDQ(strt:fnsh,b) + basinwall.surfTDQ(strt:fnsh,b) + basinwall.subTDQ(strt:fnsh,b));
        cryoYrVol(yr-1994,b)=sum(cryoholeTDQ(strt:fnsh,b));
        
        % Ablation Totals
        modelYrABL(yr-1994,b)=sum(modelABL(strt:fnsh,b));
        % Summer
        modelSmABL(yr-1994,b)=sum(modelABL(sumrstart:sumrfnsh,b));
        
        % Only use days that have obs
        % modelYrVol(yr-1994,b)=sum(modelTDQ(measdays,b));
...
if ~exist(fullPathData, 'dir')
    ...
    % Save output data
    save(path2save, 'modelTDQ', 'dates', 'modelYrVol', 'modelSmVol', 'streamYrVol', 'basinkey');
else
